...
async def dt_callback(cq: CallbackQuery, state: FSMContext):
    ...
    if act == 'd':
        ...
        data['h'] = int(val)
        await state.update_data(**data)
        log.info(f"[POST_PLAN] Selected hour: {data['h']}")
        await cq.message.edit_reply_markup(minute_kb(lang))
    elif act == 'mi':
        data['min'] = int(val)
        await state.update_data(**data)
        log.info(f"[POST_PLAN] Selected minute: {data['min']}")
        ...
        if channel == "life":
            ...
            log.info(f"[POST_PLAN] Transition to Post.select_stars for channel {channel}")
            ...
            log.info(f"[POST_PLAN] Transition to Post.wait_content for channel {channel}")
...
async def select_stars(msg: Message, state: FSMContext):
    ...
    await state.set_state(Post.wait_content)
    log.info(f"[POST_PLAN] Transition to Post.wait_content after selecting {stars} stars")
    b = InlineKeyboardBuilder()
    b.button(text='✅ Готово', callback_data='post_done')
